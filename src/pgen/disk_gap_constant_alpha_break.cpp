  ...
  for (int k=ks; k<=ke; ++k) {
    ...
    for (int j=js; j<=je; ++j) {
      ...
      for (int i=is; i<=ie; ++i) {
        ...
        // apply rotation 
        W = W_out * 0.5 * (tanh(100*(x1-1.5)) + 1.0);
        Real ur1_coef  = 0.0;      //ur1_from_Table(std::sqrt(rad*rad + z*z))*W_out;
        Real alph_rotation = 0.0;  //alph_from_Table(std::sqrt(rad*rad + z*z));
        Real beta_rotation = W;    //beta_from_Table(std::sqrt(rad*rad + z*z))*W_out;
        Real gamm_rotation = 0.0;  //gamm_from_Table(std::sqrt(rad*rad + z*z));
